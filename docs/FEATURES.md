...
│   - REST API: /api/portfolio, /api/portfolio/transactions    │
│                         │                                     │
│                         ▼                                     │
...
- `data/user_data.db` — baza de date privata. Contine portofoliul real (numar actiuni, pret achizitie) si tranzactiile.
...
1. Cloneaza repo-ul
2. Configuratorul `setup_credentials.py` va seta accesul Tradeville.
3. Ruleaza serverul pentru sincronizarea automata a bazei de date.

### Ce e safe de commit

- Fisierele `.example.json` si `.demo.json` — structura anonimizata pentru a vizualiza demo.
