...
from shared.config import SERVER_HOST, SERVER_PORT, COMPANY_DATA_FILE, PENDING_SCRAPES_FILE, SCRAPE_RESULTS_DIR, BASE_DIR, PORTFOLIO_FILE
...
def get_offline_fallback(cmd: str, symbol: str) -> dict:
    ...
    if cmd == "Symbol" and symbol:
        ...
        try:
            rows = user_conn.execute("SELECT symbol, quantity, avg_price FROM portfolio_holdings").fetchall()
            symbols = [r["symbol"] for r in rows]
            quantities = [r["quantity"] for r in rows]
            avg_prices = [r["avg_price"] for r in rows]
            # We could fetch market prices from dashboard db, but fallback might be fine with 0
            market_prices = [0 for _ in rows]
            ptypes = ["A" for _ in rows]
            return {
                "cmd": "Portfolio",
                "data": {
                    "Account": ["OFFLINE"] * len(symbols),
                    "Symbol": symbols,
                    "Quantity": quantities,
                    "AvgPrice": avg_prices,
            ...
            log.error(f"[fallback] Portfolio fallback failed: {e}")
        finally:
            user_conn.close()
...
class DashboardHandler(BaseHTTPRequestHandler):
    ...
    def _serve_static_file(self, rel_path: str):
        ...
        if ext in (".html", ".htm", ".css", ".js"):
            file_path = os.path.abspath(os.path.join(base_abs, "front-end", clean_rel))
        elif ext in (".json", ".db"):
            file_path = os.path.abspath(os.path.join(base_abs, "data", clean_rel))
        else:
        ...
        try:
            ...
            content = None
            global streamer
            if filename == "watchlist_data.json" and streamer:
                try:
            ...
            # Caching headers — eliminates massive re-downloads on page navigation
            if ext in (".css", ".js"):
        ...
        if path == "/health":
            ...
            try:
                if symbol:
                    ...
                    if comp:
                        # Limit price_history to last 90 rows — keeps fallback responses small
                        ph = comp.get("price_history")
                        if ph and len(ph) > 90:
                            comp["price_history"] = ph[-90:]
                        log.debug(f"[api] company/{symbol} returned — {len(comp.get('quarterly',[]))}Q, {len(comp.get('annual',[]))}Y, {len(comp.get('price_history',[]))}H")
            ...
            log.info("[api] GET /api/portfolio")
            data = handle_get_portfolio()
            if "error" in data:
                self._send_json(data, status=500)
            else:
                self._send_json(data)

        elif path == "/api/portfolio/transactions":
            log.info("[api] GET /api/portfolio/transactions")
            data = handle_get_transactions(limit=10)
            if "error" in data:
                self._send_json(data, status=500)
            else:
                self._send_json(data)
    ...
    def do_POST(self):
        ...
        if path == "/refresh":
            ...
            def run_sync():
                try:
                    subprocess.run([sys.executable, os.path.join(BASE_DIR, "back-end", "portfolio_updater.py"), "--force"])
                except Exception as e:
            ...
            def run_history():
                try:
                    args = [sys.executable, os.path.join(BASE_DIR, "back-end", "company_fetcher.py")]
                    if symbol:
                        args.append(symbol)
                    subprocess.run(args)
                except Exception as e:
...
def main():
    ...
    streamer.start()
    # Start initial portfolio and transaction sync in background thread
    def run_initial_sync():
        try:
            log.info("[monitor] Running initial portfolio and transaction sync in background...")
            import subprocess
            subprocess.run([sys.executable, os.path.join(BASE_DIR, "back-end", "portfolio_updater.py"), "--force"])
        except Exception as e:
            log.error(f"[monitor] Failed to run initial sync: {e}")
