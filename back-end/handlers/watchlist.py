...
def _fetch_price_for(symbol: str):
    ...
    try:
        ...
        fetcher = os.path.join(BASE_DIR, "back-end", "portfolio_updater.py")
...
def handle_add_to_watchlist(body: dict) -> dict:
    ...
    try:
        ...
        if config.streamer and config.streamer.authenticated:
            config.streamer.active_tickers.add(symbol)
            config.streamer._resubscribe()
            log.info(f"[watchlist] Added {symbol} to active websocket subscriptions")
            import threading
            threading.Thread(
                target=config.streamer._fetch_symbol_details,
                args=([symbol],),
                daemon=True
            ).start()
...
def handle_delete_from_watchlist(symbol: str) -> dict:
    ...
    try:
        import shared.config as config
        if config.streamer and symbol in config.streamer.active_tickers:
            config.streamer.active_tickers.discard(symbol)
            config.streamer._resubscribe()
