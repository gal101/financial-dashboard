...
def sync_activities(client):
    ...
    try:
        ...
        for i in range(len(dates)):
            ...
            if not dup:
                conn.execute(
                    "INSERT INTO user_transactions (date, op_type, symbol, quantity, price, commission, amount, profit, obs) "
                    "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
                    (date_val, op_type, symbol, qty, price, comm, amt, profit, obs)
                )
                inserted += 1
        conn.commit()
    except Exception as e:
